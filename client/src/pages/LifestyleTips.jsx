import React, { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import { Leaf, Flame, Droplet, BookOpen } from "lucide-react";

export default function LifestyleTips() {
  const navigate = useNavigate();
  const token = localStorage.getItem("token");

  const [tips, setTips] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedDosha, setSelectedDosha] = useState("");

  useEffect(() => {
    const fetchTips = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/lifestyle", {
          headers: { "Content-Type": "application/json" },
        });
        if (!res.ok) throw new Error("Failed to fetch lifestyle tips");
        const data = await res.json();
        setTips(data);
      } catch (err) {
        console.error(err);
        setError("Could not load lifestyle tips.");
      } finally {
        setLoading(false);
      }
    };
    fetchTips();
  }, [token]);

  if (loading) return <p className="p-10 text-center">Loading lifestyle tips...</p>;
  if (error) return <p className="p-10 text-center text-red-500">{error}</p>;
  if (tips.length === 0) return <p className="p-10 text-center">No lifestyle tips found.</p>;

  // --- Filtering ---
  const filteredTips = tips.filter((t) => {
    const matchesSearch = t.tip.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesDosha = selectedDosha ? t.doshaType === selectedDosha : true;
    return matchesSearch && matchesDosha;
  });

  const groupedTips = {
    Vata: filteredTips.filter((t) => t.doshaType === "Vata"),
    Pitta: filteredTips.filter((t) => t.doshaType === "Pitta"),
    Kapha: filteredTips.filter((t) => t.doshaType === "Kapha"),
  };

  const getDoshaIcon = (type) => {
    switch (type) {
      case "Vata":
        return <Leaf className="w-6 h-6 text-green-600" />;
      case "Pitta":
        return <Flame className="w-6 h-6 text-red-500" />;
      case "Kapha":
        return <Droplet className="w-6 h-6 text-blue-500" />;
      default:
        return <Leaf className="w-6 h-6 text-green-600" />;
    }
  };

const renderTipCards = (tipArray) =>
  tipArray.map((tip) => (
    <div
      key={tip._id}
      className="relative group bg-white p-6 rounded-2xl shadow-lg hover:shadow-2xl transition cursor-pointer flex flex-col justify-between gap-3 h-64"
    >
      <div className="flex flex-col gap-3 z-10 relative">
        <div className="flex items-center gap-3">
          {getDoshaIcon(tip.doshaType)}
          <h2 className="text-xl font-semibold text-green-700">
            {tip.tip} {/* Short heading */}
          </h2>
        </div>
        <p className="text-gray-700">{tip.description}</p> {/* Detailed description */}
      </div>

      {tip.source && (
        <div className="absolute inset-0 bg-white/70 backdrop-blur-sm p-6 rounded-2xl flex flex-col justify-center items-center opacity-0 group-hover:opacity-100 transition duration-300 z-20 hover:scale-105">
          <BookOpen className="w-8 h-8 mb-3 text-green-700" />
          <h3 className="text-lg font-semibold mb-2">Source</h3>
          <p className="text-center text-sm">{tip.source.title}</p>
        </div>
      )}
    </div>
  ));

  return (
    <div className="min-h-screen p-10 bg-green-50">
      <h1 className="text-3xl font-bold text-green-900 mb-8 text-center">
        Lifestyle Tips
      </h1>

      {/* Search + Filter */}
      <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-10">
        {/* Search Input */}
        <div className="relative w-full sm:w-1/3">
          <input
            type="text"
            placeholder="Search lifestyle tips..."
            className="w-full p-3 pl-10 rounded-full bg-green-100 text-green-900 placeholder-green-700 shadow-inner focus:bg-green-200 focus:ring-2 focus:ring-green-600 outline-none transition"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth={2}
            stroke="currentColor"
            className="absolute left-3 top-3.5 w-5 h-5 text-green-800"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M21 21l-4.35-4.35m0 0A7.5 7.5 0 104.5 4.5a7.5 7.5 0 0012.15 12.15z"
            />
          </svg>
        </div>

        {/* Dosha Dropdown */}
        <div className="relative w-full sm:w-1/4">
          <select
            className="w-full p-3 pl-10 rounded-full bg-green-100 text-green-900 shadow-inner 
             focus:bg-green-200 focus:ring-2 focus:ring-green-600 
             outline-none transition cursor-pointer appearance-none hover:bg-green-200"
            value={selectedDosha}
            onChange={(e) => setSelectedDosha(e.target.value)}
          >
            <option value="">All Doshas</option>
            <option value="Vata">Vata</option>
            <option value="Pitta">Pitta</option>
            <option value="Kapha">Kapha</option>
          </select>

          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth={2}
            stroke="currentColor"
            className="absolute right-3 top-3.5 w-5 h-5 text-green-800 pointer-events-none"
          >
            <path strokeLinecap="round" strokeLinejoin="round" d="M8 10l4 4 4-4" />
          </svg>
        </div>
      </div>

      {/* Tips by Dosha */}
      {["Vata", "Pitta", "Kapha"].map((dosha) => (
        groupedTips[dosha].length > 0 && (
          <section key={dosha} className="mb-10">
            <h2 className="text-2xl font-semibold text-green-800 mb-4">{dosha} Tips</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {renderTipCards(groupedTips[dosha])}
            </div>
          </section>
        )
      ))}

      {filteredTips.length === 0 && (
        <p className="text-center text-gray-500 mt-10">
          No lifestyle tips match your search/filter.
        </p>
      )}

      <div className="mt-10 text-center">
        <button
          onClick={() => navigate("/dashboard")}
          className="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition"
        >
          Back to Dashboard
        </button>
      </div>
    </div>
  );
}